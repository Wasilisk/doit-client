...
import { profileSchema, type ProfileSchema } from '~/types/schemas/profileSchema'
...
<template>
    <div class="p-4 md:p-6 lg:p-8 flex justify-center">
        <div class="w-full max-w-2xl bg-white rounded-xl shadow border border-gray-100 overflow-hidden">
            <div class="p-6 md:p-8">
                <h1 class="text-2xl font-bold text-gray-800 mb-8">{{ t('profile.title') }}</h1>
                ...
                <div v-if="user" class="flex flex-col md:flex-row gap-8 items-start">
                    <div class="flex-shrink-0 relative group">
                        ...
                        >
                            ...
                            >
                                <i v-if="mutation.isPending.value" class="pi pi-spinner pi-spin text-3xl"></i>
                                <template v-else>
                                    <i class="pi pi-camera text-3xl mb-2"></i>
                                    <span class="text-sm font-medium">{{ t('profile.actions.uploadAvatar') }}</span>
                                </template>
                            </div>
                        </div>
                        <input 
                    ...
                    <div class="flex-grow flex flex-col gap-6 w-full">
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">{{ t('profile.fullName') }}</label>
                            <div class="flex items-center gap-3 w-full">
                                <template v-if="isEditingName">
                                    <form @submit.prevent="handleSaveName" class="flex items-start gap-3 w-full">
                                        <div class="flex-grow">
                                            <AuthFormField 
                                ...
                                <template v-else>
                                    <p class="text-lg font-semibold text-gray-800 flex-grow">{{ user.full_name }}</p>
                                    <Button 
                                        icon="pi pi-pencil" 
                                        severity="secondary" 
                                        text 
                        ...
                        <div>
                            ...
                            <p class="text-lg text-gray-600 bg-gray-50 p-3 rounded-lg border border-gray-100 flex items-center gap-2">
